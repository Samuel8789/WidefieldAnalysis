# -*- coding: utf-8 -*-
"""
Created on Thu Feb  6 10:19:42 2025
...
mask_shared_folder = rf'C:\Users\sp3660\Documents\Projects\LabNY\Amsterdam\Analysis\TempMovies\Animal{mouse}'
...
gabor_mask4 = cv2.imread(image_folder+r'\gabor_mask_Binocular.png') # The size of all inner,middle, outer gabor images have to be same.
...
gray_gabor_mask4 = cv2.cvtColor(gabor_mask4, cv2.COLOR_BGR2GRAY) # The size of all inner,middle, outer gabor images have to be same.
...
ax.flatten()[4].imshow(gray_gabor_mask1+gray_gabor_mask2+gray_gabor_mask3+gray_gabor_mask4)
...
def get_roi_coordinates(low_mag_input, roi_input,roi_name,whole_name, match_to_use=10, eps=10, min_samples=2, angle_deviation_threshold=2, manual=None):
    ...
    if manual:  # Manual matching mode
        ...
        if not affine_matrix.any() or choice:
            # Show both images and allow manual point selection
            cv2.imshow('Low Magnification Image', low_mag)
            cv2.setMouseCallback('Low Magnification Image', select_points, param='image1')
...
if affine_matrix.any():
    choice = int(input("Do you want to redo the transform for map and gabor (1 for yes 0 for no): ")) 
if not affine_matrix.any() or choice:
...
#%% MAPPING ROIS TO WHOLE BRAIN IMAHES
...
map_roi_coordinates = get_roi_coordinates(
    gray_mapr_ref, gray_map_roi,'map_roi','map_whole', match_to_use=None, eps=10, min_samples=2, 
     angle_deviation_threshold=2, manual=True)
#%% REGISTERING ROI COORDINATES ACROSS DAYS
...
gabor4_mask_full_fov=get_translated_mask_coordinates(gabor_roi_coordinates.reshape(-1, 1, 2), gray_gabor_mask4)
...
registered_gabor3_mask_full_fov=cv2.transform(gabor3_mask_full_fov.reshape(-1, 1, 2), affine_matrix)
registered_gabor4_mask_full_fov=cv2.transform(gabor4_mask_full_fov.reshape(-1, 1, 2), affine_matrix)
...
cv2.drawContours(gabor_reference_with_roi, [registered_gabor4_mask_full_fov], -1, (255, 0, 0), 2)
...
cv2.drawContours(common_reference, [registered_gabor4_mask_full_fov], -1, (255, 0, 0), 2)
...
#%% THIS ONLY NEEDS PROJECTED GRID
...
if proj:   
    ...
    if gray_proj_ref.any() and gray_mapr_ref.any() :
        ...
        if affine_matrix2.any():
            choice = int(input("Do you want to redo the transform for exp and map (1 for yes 0 for no): ")) 
        if not affine_matrix2.any() or choice:
    ...
    reregistered_gabor4_mask_full_fov= cv2.transform(registered_gabor4_mask_full_fov.astype(np.float32).reshape(-1, 1, 2), affine_matrix2)
    ...
    cv2.drawContours(final_reference, [np.int32(reregistered_gabor4_mask_full_fov)], -1, (255, 0, 0), 2)
    ...
    #%% THIS NEEDS WOHLE BRIN IMAGE OF EXPERIMENTAL DAY
    ...
    final_gabor4_mask_coordinates=reregistered_gabor4_mask_full_fov.squeeze()
    ...
    binary_mask_gabor4 = np.zeros((height, width), dtype=np.uint8)
    ...
    cv2.fillPoly(binary_mask_gabor4, [np.int32(final_gabor4_mask_coordinates)], color=(255, 255, 255))
    ...
    final_region4 = cv2.subtract(binary_mask_map, binary_mask_gabor4)
    ...
    final_mask_v1_nogabor4 = map_fov_to_projector(final_region4, tps, A)
    ...
    ax[1, 3].imshow(final_mask_v1_nogabor4, cmap='gray')
    ax[1, 3].set_title('V1 - Gabor 3')
    ...
    cv2.imwrite(os.path.join(mask_shared_folder, 'final_mask_gabor_inner.jpg'), final_mask_gabor1)
    cv2.imwrite(os.path.join(mask_shared_folder, 'final_mask_gabor_middle.jpg'), final_mask_gabor2)
    cv2.imwrite(os.path.join(mask_shared_folder, 'final_mask_gabor_outer.jpg'), final_mask_gabor3)
    cv2.imwrite(os.path.join(mask_shared_folder, 'final_mask_fullv1.jpg'), final_mask_fullv1)
    cv2.imwrite(os.path.join(mask_shared_folder, 'final_mask_v1_no_gabor_inner.jpg'), final_mask_v1_nogabor1)
    cv2.imwrite(os.path.join(mask_shared_folder, 'final_mask_v1_no_gabor_middle.jpg'), final_mask_v1_nogabor2)
    cv2.imwrite(os.path.join(mask_shared_folder, 'final_mask_v1_no_gabor_outer.jpg'), final_mask_v1_nogabor3)
    cv2.imwrite(os.path.join(mask_shared_folder, 'final_mask_v1_no_gabor_binocular.jpg'), final_mask_v1_nogabor4)
    
    
    print(f"Masks saved to directory: {mask_shared_folder}")
